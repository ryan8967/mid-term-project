...
export default {
  ...
  methods: {
    fetchProducts() {
      // const productId = this.$route.params.id; // 获取路由参数中的seller id
      const productId = this.$route.params.id; // Debugging line
      console.log("productId:", productId); // Debugging line
      // let token = localStorage.getItem("jwtToken");
      // const url = `http://127.0.0.1:8000/api/myproducts`;
      const url = `http://localhost:8000/api/products/${productId}/seller/products`;
      ...
      axios
        ...
        .then((response) => {
          ...
          this.products = Array.isArray(response.data) ? response.data : []; // 确保 products 是数组
        })
        .catch((error) => {
          console.error(
